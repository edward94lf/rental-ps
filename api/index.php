<?php
/**
 * RENTAL FADJAR - VERSI PENIPU WAKTU (AUTO-SINKRON)
 */
...
function panggil_tuya($url, $method, $body = "", $token = "") {
    ...
    // --- JURUS SAKTI: TANYA JAM KE SINGAPORE DULU ---
    ...
    // Pakai jam dari Tuya (hasilnya dalam milidetik)
...
if (isset($_GET['aksi'])) {
    ...
    if (isset($resToken['result']['access_token'])) {
        ...
        if (isset($resCmd['success']) && $resCmd['success']) {
            ...
            $pesan = "GAGAL PERINTAH: " . ($resCmd['msg'] ?? "Ditolak Singapore");
        }
    } else {
        $msgToken = isset($resToken['msg']) ? $resToken['msg'] : "Jam Gak Sinkron";
